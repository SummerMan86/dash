...
| `2026-04-05` | `schema change` | AUTH-5 fix: расширен CHECK `emis.audit_log.entity_type` значением `user_account`, чтобы audit contract покрывал admin CRUD по пользователям (`/api/emis/admin/users/*`) тем же append-only журналом, что и остальные write-side операции.                                                                                                      | `db/current_schema.sql`, `docs/emis_access_model.md`, `RUNTIME_CONTRACT.md` |

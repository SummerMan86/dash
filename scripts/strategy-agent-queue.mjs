...
function usage() {
	console.log(`Usage:
  node ./scripts/strategy-agent-queue.mjs seed --run-id <id> [--run-label <label>] [--source-root <path>] [--workbook-version <version>] [--batch-id <id>]
  node ./scripts/strategy-agent-queue.mjs claim --run-id <id> --agent-code <code> [--agent-label <label>] [--lease-minutes <minutes>]
  node ./scripts/strategy-agent-queue.mjs complete --task-id <uuid> --agent-code <code> --result-status <completed|blocked|skipped> [--note <text>]
  node ./scripts/strategy-agent-queue.mjs intake-insert --spec <path-to-json>
  node ./scripts/strategy-agent-queue.mjs status [--run-id <id>] [--limit <n>] [--full]`);
}
...
async function seed(client, options) {
	if (!options['run-id']) {
		throw new Error('--run-id is required for seed');
	}

	const result = await client.query(
		`SELECT stg_strategy.strategy_seed_parse_run($1, $2, $3, $4, $5) AS seeded_count`,
		[
			options['run-id'],
			options['run-label'] ?? null,
			options['source-root'] ?? null,
			options['workbook-version'] ?? 'hybrid_v7_1',
			options['batch-id'] ?? null
		]
	);
		...
		JSON.stringify(
			{
				...
				seededCount: Number(result.rows[0]?.seeded_count ?? 0)
			},
			null,
			2
		)
...
async function claim(client, options) {
	if (!options['run-id']) {
		throw new Error('--run-id is required for claim');
	}
	if (!options['agent-code']) {
		throw new Error('--agent-code is required for claim');
	}

	const result = await client.query(
		`SELECT * FROM stg_strategy.strategy_claim_parse_task($1, $2, $3, $4)`,
		[
			options['run-id'],
			options['agent-code'],
			options['agent-label'] ?? null,
			Number(options['lease-minutes'] ?? 120)
		]
	);
	...
	if (!result.rows.length) {
		console.log(
			JSON.stringify(
				{ runId: options['run-id'], task: null, message: 'No available task' },
				null,
				2
			)
		);
...
async function complete(client, options) {
	if (!options['task-id']) {
		throw new Error('--task-id is required for complete');
	}
	if (!options['agent-code']) {
		throw new Error('--agent-code is required for complete');
	}
	if (!options['result-status']) {
		throw new Error('--result-status is required for complete');
	}

	const result = await client.query(
		`SELECT * FROM stg_strategy.strategy_complete_parse_task($1::uuid, $2, $3, $4)`,
		[options['task-id'], options['agent-code'], options['result-status'], options.note ?? null]
	);

	console.log(JSON.stringify(result.rows[0] ?? null, null, 2));
}

async function loadIntakeSpec(specPath) {
	if (!specPath) {
		throw new Error('--spec is required for intake-insert');
	}
...
async function upsertDocument(client, document) {
	await client.query(
		`INSERT INTO stg_strategy.strategy_document_intake (
...
async function upsertFact(client, fact) {
	await client.query(
		`INSERT INTO stg_strategy.strategy_fact_intake (
...
async function upsertLink(client, link) {
	await client.query(
		`INSERT INTO stg_strategy.strategy_link_intake (
...
async function upsertGap(client, gap) {
	await client.query(
		`INSERT INTO stg_strategy.strategy_gap_intake (
...
async function intakeInsert(client, options) {
	const spec = await loadIntakeSpec(options.spec);
	...
	try {
		...
		const verify = await client.query(
			`SELECT
				(SELECT count(*) FROM stg_strategy.strategy_document_intake WHERE run_id = $1 AND source_document_code = $2) AS document_rows,
				(SELECT count(*) FROM stg_strategy.strategy_fact_intake WHERE run_id = $1 AND doc_id = $3) AS fact_rows,
				(SELECT count(*) FROM stg_strategy.strategy_link_intake WHERE run_id = $1 AND source_document_code = $2) AS link_rows,
				(SELECT count(*) FROM stg_strategy.strategy_gap_intake WHERE run_id = $1 AND doc_id_1 = $3) AS gap_rows`,
...
async function status(client, options) {
	if (options['run-id']) {
		...
		const summary = await client.query(
			`SELECT * FROM mart_strategy.strategy_parse_run_summary WHERE run_id = $1`,
			[options['run-id']]
		);
		const queue = await client.query(
			`SELECT *
			 FROM mart_strategy.strategy_parse_task_queue
			 WHERE run_id = $1
			 ORDER BY
			 	CASE priority
			 		WHEN 'P0' THEN 0
			 		...
			 		ELSE 4
			 	END,
			 	source_document_file NULLS LAST,
			 	source_document_name
			 ${full ? '' : 'LIMIT $2'}`,
			full ? [options['run-id']] : [options['run-id'], limit]
		);
		console.log(
			JSON.stringify(
				{
					summary: summary.rows[0] ?? null,
					tasksReturned: queue.rows.length,
					tasks: queue.rows
				},
				null,
				2
	...
	const result = await client.query(
		`SELECT *
		 FROM mart_strategy.strategy_parse_run_summary
...
async function main() {
	...
	if (!command || !['seed', 'claim', 'complete', 'intake-insert', 'status'].includes(command)) {
		usage();
		process.exitCode = 1;
		return;
	}
	...
	try {
		switch (command) {
			case 'seed':
				...
				break;
			case 'claim':
				await claim(client, options);
				break;
			case 'complete':
				await complete(client, options);
				break;
			case 'intake-insert':
				await intakeInsert(client, options);
				break;
			case 'status':
				await status(client, options);
				break;
		}
	} finally {
		await client.end();

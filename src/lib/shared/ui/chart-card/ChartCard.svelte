...
>
	...
	<div class="flex items-center justify-between px-6 pt-5 pb-2">
		...
		{#if updatedAt}
			<span class="type-badge rounded-full bg-muted px-2.5 py-1 text-muted-foreground">
				{updatedAt}
			</span>
		{/if}
	</div>
	...
	<div class={cn('p-6 pt-0', fluid && 'flex min-h-0 flex-1 flex-col')}>
		{#if loading}
			...
			<div class={cn('flex items-end gap-2 px-4', fluid ? 'min-h-[200px] flex-1' : 'h-72')}>
				{#each skeletonHeights as height}
					<div class="skeleton-shimmer flex-1 rounded" style="height: {height}%"></div>
				{/each}
			</div>
		{:else}

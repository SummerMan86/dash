...
Неразрешённый docs `WARNING` на slice level автоматически поднимается до `CRITICAL` при wave closure только для contract-touching docs: `RUNTIME_CONTRACT.md`, `db/schema_catalog.md`, `db/current_schema.sql`, новые инварианты в `docs/agents/invariants.md` и overlay-файлах типа `invariants-emis.md`.
Navigation `AGENTS.md` drift может carry over до wave closure как `WARNING`, если в handoff/report зафиксированы owner и expiry.

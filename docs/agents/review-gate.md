...
- любой slice, который пишет product code, требует хотя бы одного fresh reviewer pass от агента, который этот код не писал;
...
- пользователь или `orchestrator` могут снизить coverage только сверх этого минимума; полностью waiv'ить независимый review для code change нельзя.
...
- `trivial` не является причиной skip, если slice пишет product code.
...
Если change пишет product code, хотя бы один независимый reviewer pass остаётся обязательным даже при skip полного Review Gate.

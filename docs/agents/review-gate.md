...
| --- | --- | --- |
| Planned scope с architectural surface | `lead-tactical` | `architecture-reviewer` (audit mode) |
...
2. `lead-tactical` эскалирует finding к `lead-strategic` (или запускает `architecture pass` §3.1).
...
- `lead-tactical` — перед execution, когда bounded self-check недостаточен (`workflow.md` §2.3.1);

...
После выпуска минимального platform upgrade по фильтрам часть EMIS-слайса из этого spec уже закрыта вне исходной очередности.

Что уже реализовано:

- platform filter runtime усилен без полного rewrite:
  - `useFilterWorkspace(...)`
  - `shared/workspace/owner` scopes
  - `sharedKey/urlKey/registrationKey`
  - namespaced URL sync
  - target-aware planner
- thin transport endpoints:
  - `GET /api/emis/search/objects`
  - `GET /api/emis/search/news`
- `/emis` больше не является только foundation-page:
  - есть общий filter workspace для map/list
  - `layer=objects|news|all`
  - results list для objects/news
  - map wiring через общие planner targets
  - endpoint-backed options для EMIS dictionaries
- DB foundation уже продвинут дальше исходной первой волны:
  - локальный PostGIS dev runtime через Docker Compose на `5435`
  - `008_emis_identity_and_provenance.sql`
  - `009_emis_audit_log.sql`
  - `010_emis_mart_views.sql`
  - `011_emis_ship_route_vessels.sql`
- BI/read-side contract уже не только планируется, а частично работает:
  - `mart.emis_news_flat`
  - `mart.emis_object_news_facts`
  - `mart.emis_objects_dim`
- ship-route slice уже встроен в `/emis` как реальный thin vertical slice:
  - datasets `emis.ship_route_vessels`
  - datasets `emis.ship_route_points`
  - datasets `emis.ship_route_segments`
  - route preview на карте и в summary panel
  - `shipHbkId` как workspace filter с URL sync
  - dictionary endpoint `/api/emis/dictionaries/vessels`

Почему это важно:

- эти изменения были вытянуты вперед относительно исходных Wave B / Wave C;
- причиной была необходимость закрыть реальный cross-filter runtime для BI + EMIS;
- исходный порядок в документе сохраняется как roadmap, но статус ниже нужно читать с учетом этой пометки.
...
На текущем этапе базовой конвенцией считаем плоские префиксы:
...
- упрощает будущий физический split в монорепо.
...
Но переход к вложенной структуре имеет смысл только как отдельный осознанный refactor, когда EMIS-домен заметно вырастет. Для MVE и ранних post-MVE волн текущий default - сохранять плоскую `emis-*` схему.
...
- GIST
- FTS
- active partial unique indexes для canonical identity с учетом `deleted_at IS NULL`
...
#### Следующая обязательная migration wave после foundation

До production-shaped MVE нужно добавить отдельную migration wave для:
...
#### Текущий фактический migration checkpoint в репозитории

На 17 марта 2026 года уже реализовано и проверено:

- `001-011` применяются green через локальный dev runtime;
- `008` добавляет identity/provenance contract и partial unique indexes;
- `009` добавляет append-only `emis.audit_log`;
- `010` создает `mart.emis_*` BI views;
- `011` создает `mart.emis_ship_route_vessels` как стабильный vessel catalog поверх `mart_emis`.
...
- `search/objects` и `search/news` уже реализованы как тонкий transport над существующими query modules;
- map endpoints уже используются текущим `/emis` workspace;
- полное query hardening по сортировкам, smoke-тестам и унификации conventions остается отдельной задачей.
...
Status update от 16 марта 2026:

- lifecycle stabilization уже сделан в platform layer;
- текущий рабочий путь теперь:
...
- app-shared subset v1 уже работает для `dateRange` между Wildberries и EMIS;
- fallback с отдельным локальным store внутри EMIS workspace больше не является рекомендуемым направлением.
...
- отдельной задачей остаются query hardening, cross-route smoke coverage и выравнивание conventions.
...
- переключение результата между objects/news пока остается локальным UI state;
- переходы в отдельные карточки пока не закрыты, потому что catalog/detail routes еще не реализованы.
...
- минимальный slice из Wave B уже выполнен раньше исходной очереди;
- минимальный slice из Wave C тоже уже выполнен раньше исходной очереди;
- фактический ближайший фокус теперь смещается к:
  - catalogs/detail pages
  - audit/provenance scaffold
  - BI wiring
  - query/runtime hardening
...
- server layer для dictionaries, objects, news, links, ship-routes;
- list/detail endpoints для objects и news;
- search endpoints для objects и news;
- map endpoints и `EmisMap.svelte`;
- health endpoint и базовая `/emis` page;
- workspace filters и общий map/list runtime на `/emis`;
...
- catalogs/detail routes еще не реализованы;
- `/emis` уже рабочий, но пока без переходов в отдельные карточки;
...
2. Дожать query layer: единые conventions, smoke checks, limit/offset/sort consistency.
3. Добавить каталоги и карточки как отдельные entry points.
4. После этого ввести manual create/edit flows и заготовку audit attribution.
5. Завершить BI wiring и smoke tests как условие первой рабочей поставки.

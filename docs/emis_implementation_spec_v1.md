...
Текущий статус репозитория, закрытые slice'ы и ближайший focus теперь вынесены в отдельный master doc:
[EMIS Session Bootstrap](./emis_session_bootstrap.md).

Этот spec фиксирует только implementation decisions, structure и rollout order.
...
- порядок выше сохраняется как rollout roadmap;
- реальный статус репозитория может быть уже дальше отдельных волн;
- актуальный status и practical focus смотреть в `docs/emis_session_bootstrap.md`.
  - `/emis` edge-case UX polish
...
- Этот документ фиксирует implementation decisions, структуру каталогов, API shape и rollout order.
- Текущее состояние репозитория и ближайший focus смотреть в [EMIS Session Bootstrap](./emis_session_bootstrap.md).
- Runtime/API conventions смотреть в `src/lib/server/emis/infra/RUNTIME_CONTRACT.md`.

...
Architecture for the BI vertical. Wave 1 (BR-1..BR-10) is implemented and merged.
The stable core described here is now the current architecture, not just a target.
For pre-refactor legacy state see [architecture_dashboard_bi.md](./architecture_dashboard_bi.md).

## Scope

- Covers: target dataset runtime, target filter wire contract, IR policy, client-side query model, provider extension path, schema introspection
...
Read BI architecture at four levels:

1. [architecture.md](./architecture.md) — repo-wide foundation
2. [architecture_dashboard_bi.md](./architecture_dashboard_bi.md) — current-state BI vertical
3. this document — target-state BI architecture
4. `docs/plans/bi_refactor_rollout.md` — execution order, slices, defaults, and acceptance criteria

## Sections

1. [Target Dataset Runtime](#1-target-dataset-runtime) — registry, providers, execution, errors, observability
2. [Target Filter Wire Contract](#2-target-filter-wire-contract) — flat params, planner, scope/apply tiers, migration
...
5. [Provider and Backend Extension Path](#5-provider-and-backend-extension-path) — new datasets, new providers, lifecycle, testing

## High-Level Vision

The target BI architecture is built on two organizing principles:
...
This document defines the durable target for BI/read-side refactoring. It is both the design reference and a description of the current implemented architecture for the stable core.
...
### Target: One Canonical `DatasetRegistryEntry`
...
This is the target contract for an embedded query builder in low-ops deployments.
...
- current planner supports more scope/apply combinations than the first-wave BI target actually needs
...
The first-wave BI target explicitly narrows the supported combinations.
...
Deferred, not first-wave target:
...
For metrics such as counts, sums, averages, rejection rates, or participant/service groupings, the first-wave BI target does not compile raw-table aggregation into `SelectIr`.
...
- provider cache key includes dataset id + normalized params + relevant server context
...
## 5. Provider and Backend Extension Path
...
Detailed execution slices, test matrices, and rollout sequencing live outside this document in `docs/plans/bi_refactor_rollout.md`.

---

## Read Next

- [architecture_dashboard_bi.md](./architecture_dashboard_bi.md) -- BI current state
- [architecture.md](./architecture.md) -- repo-wide foundation
- `docs/plans/bi_refactor_rollout.md` -- implementation slices and rollout defaults
